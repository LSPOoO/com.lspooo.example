package com.lspooo.example.plugin.common.common.bar;
...
public class StatusBarCompat {
    ...
    /**
     * 是否需要半透明化状态栏
     * @return 半透明化状态栏（不设置纯白色）
     */
    public static boolean isAlphaStatusBar() {
        return isZTE() || isVivo() || isOppo() || isZhuoYue();
    }
    ...
    public static boolean isZTE() {
        return (Build.BRAND.equalsIgnoreCase("ZTE") && Build.VERSION.SDK_INT < Build.VERSION_CODES.M);
    }
    ...
    public static boolean isVivo() {
        return (Build.BRAND.equalsIgnoreCase("Vivo") && Build.VERSION.SDK_INT < Build.VERSION_CODES.M);
    }
    ...
    public static boolean isOppo() {
        return (Build.BRAND.equalsIgnoreCase("Oppo") && Build.VERSION.SDK_INT < Build.VERSION_CODES.M);
    }
     ...
     * @return 判断是否TCL5.0手机
     */
    public static boolean isZhuoYue() {
        return (Build.BRAND.equalsIgnoreCase("TCL") && Build.VERSION.SDK_INT < Build.VERSION_CODES.M);
    }
    ...
    public static void setStausBarTextColor(Activity activity,boolean isLightColor) {
        if ((activity.getWindow().getAttributes().flags & WindowManager.LayoutParams.FLAG_FULLSCREEN) > 0
                || StatusBarExclude.exclude) {
            return;
        }
        LightStatusBarCompat.setLightStatusBar(activity.getWindow(), isLightColor);
    }
