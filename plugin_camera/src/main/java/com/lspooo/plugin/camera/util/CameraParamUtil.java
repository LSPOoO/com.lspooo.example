package com.lspooo.plugin.camera.util;
...
public class CameraParamUtil {
    ...
    public List<CharSequence> getPictureSize(List<Camera.Size> list, float rate) {
        Collections.sort(list, sizeComparator);
        List<CharSequence> mSize  = new ArrayList<>();
        for (Camera.Size s : list) {
            if (equalRate(s, rate)) {
                Log.i(TAG, "MakeSure Picture :w = " + s.width + " h = " + s.height);
                mSize.add(s.height + "*" + s.width);
            }
        }
        return mSize;
    }
